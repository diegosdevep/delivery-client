...
import { fetchOrdenByIdRealTime } from '../../../redux/ordersReducer';

const LastTicketScreen = () => {
  ...
  console.log(order);
  useEffect(() => {
    dispatch(fetchOrdenByIdRealTime(orderId));
  }, [dispatch, orderId]);

  const platillos = order?.orden?.map((orden) => orden.platillo);
    ...
    <SafeAreaView style={{ backgroundColor: 'white', flex: 1 }}>
      ...
      {platillos?.map((platillo) => (
        <View key={platillo?.id}>
          <Text>Platillo:</Text>
          <Text>Categoria: {platillo?.categoria}</Text>
...
export default LastTicketScreen;
